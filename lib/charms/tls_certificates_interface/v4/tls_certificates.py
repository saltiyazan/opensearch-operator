...
class CertificateSigningRequest:
    ...
    has_unique_id_in_non_critical_extension: bool = False
    ...
    def from_string(cls, csr: str) -> "CertificateSigningRequest":
        ...
        try:
            csr_object.extensions.get_extension_for_oid(NameOID.X500_UNIQUE_IDENTIFIER)
            has_unique_id_in_non_critical_extension = True
        except x509.ExtensionNotFound:
            has_unique_id_in_non_critical_extension = False
        try:
        ...
        return cls(
            ...
            has_unique_id_in_non_critical_extension=bool(has_unique_id_in_non_critical_extension),
        )
...
class CertificateRequestAttributes:
    ...
    add_unique_id_to_non_critical_extension: bool = False
    ...
    def generate_csr(
        ...
        return generate_csr(
            ...
            add_unique_id_to_non_critical_extension=self.add_unique_id_to_non_critical_extension,
        )
    ...
    def from_csr(cls, csr: CertificateSigningRequest, is_ca: bool):
        ...
        return cls(
            ...
            add_unique_id_to_non_critical_extension=csr.has_unique_id_in_non_critical_extension,
        )
...
def generate_csr(  # noqa: C901
    ...
    if add_unique_id_to_non_critical_extension:
        unique_id = uuid.uuid4()
        csr = csr.add_extension(
            x509.UnrecognizedExtension(x509.NameOID.X500_UNIQUE_IDENTIFIER, unique_id.bytes),
            critical=False,
        )
            ...
            if provider_certificate.certificate_signing_request in csrs:
                ...
                if provider_certificate.revoked:
                    with suppress(SecretNotFoundError):
                        ...
                        if secret_content.get("certificate", "") == str(
                            provider_certificate.certificate
                        ):
                            logger.debug(
                                "Secret has the revoked certificate. Removing it."
                            )
                            secret.remove_all_revisions()
                else:
...
class TLSCertificatesProvidesV4(Object):
    ...
    def _configure(self, _: EventBase) -> None:
        ...
        self._remove_expired_certificates()
    ...
    def _remove_expired_certificates(self) -> None:
        provider_certificates = self.get_provider_certificates()
        for provider_certificate in provider_certificates:
            if provider_certificate.certificate.expiry_time < datetime.now():
                self._remove_provider_certificate(
                    certificate=provider_certificate.certificate,
                )
